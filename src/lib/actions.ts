...
import { productsTable, categoriesTable, salesTable, stockMovementsTable, productVariantsTable, purchaseOrdersTable, purchaseOrderItemsTable, cashRegisterSessionsTable, usersTable, discountLogsTable } from "@/db";
import { eq, ilike, and, or, sql, desc, isNull, inArray } from "drizzle-orm";
...
// -------------------------------------------------------------
// BULK DISCOUNT (DÉSTOCKAGE EN LOT)
// -------------------------------------------------------------
...
export async function applyBulkDiscount(
  ...
  if (userRole !== "admin") throw new Error("Seuls les administrateurs peuvent appliquer des remises de déstockage.");
  ...
  // Appliquer la remise + enregistrer les logs dans une transaction
  await db.transaction(async (tx) => {
    for (const product of products) {
      const priceBefore = Number(product.unitSalePrice ?? 0);
      const priceAfter =
        Math.round(priceBefore * (1 - discountPercent / 100) * 100) / 100;

      // Mise à jour du prix de vente
      await tx
        .update(productsTable)
        .set({ unitSalePrice: String(priceAfter), updatedAt: new Date() })
        .where(eq(productsTable.id, product.id));

      // Log de la remise
      await tx.insert(discountLogsTable).values({
        productId: product.id,
        productName: product.name,
        discountPercent: String(discountPercent),
        priceBeforeDiscount: String(priceBefore),
        priceAfterDiscount: String(priceAfter),
        appliedBy: userId,
        batchId,
      });
    }
  ...
  return { success: true, count: products.length, batchId };
}

export async function getDiscountLogs(limit = 100) {
  return await db
    .select({
      id: discountLogsTable.id,
      productId: discountLogsTable.productId,
      productName: discountLogsTable.productName,
      discountPercent: discountLogsTable.discountPercent,
      priceBeforeDiscount: discountLogsTable.priceBeforeDiscount,
      priceAfterDiscount: discountLogsTable.priceAfterDiscount,
      batchId: discountLogsTable.batchId,
      appliedByName: usersTable.name,
      createdAt: discountLogsTable.createdAt,
    })
    .from(discountLogsTable)
    .leftJoin(usersTable, eq(discountLogsTable.appliedBy, usersTable.id))
    .orderBy(desc(discountLogsTable.createdAt))
    .limit(limit);
}
